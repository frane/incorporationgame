...
function initCoffees(g){
  g.coffees=[];
  const spots=[[12,9],[34,9],[58,9],[20,18],[47,18],[68,18],[7,30],[72,30],[40,44],[15,56],[52,56],[68,56],[30,64],[55,64]];
...
CV.addEventListener('click',e=>{
  ...
  if(G.scene==='over'){ resetToTitle(); return; }
  if(G.notesFor){ G.notesFor=null; return; }
  if(G.scene==='win'){
    if(G.winLock>0) return;
    const pt=canvasPoint(e);
    if(pt.y>H-46){
      const row=pt.y>H-26?1:0;
      if(row===G.winSel) winConfirm(); else { G.winSel=row; sfx.step(); }
    }
    return;
  }
  if(G.dlg){
